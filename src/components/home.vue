...
export default {
  ...
  methods: {
    createMatch: function () {
      ...
      this.$http.post('http://magictg-api.azurewebsites.net/1/mtg-games/1/mtg-games', this.host).then((res) => {
        this.host = ''
    ...
    startDuel: function(gameOn, id) {
      ...
      if(!gameOn) {
        match.Game_On = true
        this.$http.put('http://magictg-api.azurewebsites.net/1/mtg-games/1/mtg-games', match).then((res) => {
          let data = JSON.parse(res.body)
  ...
  ready () {
    let vm = this
    this.$http.get('http://magictg-api.azurewebsites.net/1/mtg-games/1/mtg-games').then((res) => {
      let data = JSON.parse(res.body)
      data.forEach((host) => {
