      ...
      .mdl-card__supporting-text
        ...
        form(action='#')
          .mdl-textfield.mdl-js-textfield.mdl-textfield--floating-label
            input#sample3.mdl-textfield__input(type='text'  v-model='host.Host')
            label.mdl-textfield__label(for='sample3') Nickname
...
export default {
  data: function () {
    return {
      host: {Host: '', Structure: 'Single Game'},
      hostedMatches: []
    }
  },
  methods: {
    createMatch: function () {
      ...
      this.$http.post('https://magictg-api.azurewebsites.net/1/mtg-games/1/mtg-games', this.host).then((res) => {
        this.host = ''
    ...
    startDuel: function(gameOn, id) {
      let match = {
        Id: id,
        Opponents: 'DuelistBot'
      }
      if(!gameOn) {
        match.Game_On = true
        this.$http.put('https://magictg-api.azurewebsites.net/1/mtg-games/1/mtg-games', match).then((res) => {
          let data = JSON.parse(res.body)
  ...
  ready () {
    let vm = this
    this.$http.get('https://magictg-api.azurewebsites.net/1/mtg-games/1/mtg-games').then((res) => {
      let data = JSON.parse(res.body)
      data.forEach((host) => {
