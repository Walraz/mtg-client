...
export default {
  ...
  methods: {
    createMatch: function () {
      ...
      this.$http.post('https://young-meadow-68232.herokuapp.com/1/mtg-games', this.host).then((res) => {
        this.host = ''
    ...
    startDuel: function(gameOn, id) {
      ...
      if(!gameOn) {
        match.Game_On = true
        this.$http.put('https://young-meadow-68232.herokuapp.com/1/mtg-games', match).then((res) => {
          let data = JSON.parse(res.body)
  ...
  ready () {
    let vm = this
    this.$http.get('https://young-meadow-68232.herokuapp.com/1/mtg-games').then((res) => {
      let data = JSON.parse(res.body)
      data.forEach((host) => {
