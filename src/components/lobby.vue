...
export default {
  ...
  ready () {
    ...
    this.$http.get(URL.API+'/1/mtg-games').then((res) => {
      let data = JSON.parse(res.body)
      data.forEach((host) => {
        host.Time_Created = moment.utc(host.Time_Created).format('YYYY-MM-DD HH:mm:ss+00:00')
    ...
    socket.on('join match', function(match) {
      ...
      vm.hostedMatches[id].Time_Created = moment.utc(match.Time_Created).format('YYYY-MM-DD HH:mm:ss+00:00')
